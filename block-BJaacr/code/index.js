...
for (const people of got.houses[0].people) {
  console.log(people.name, "starks");
}
// 5. Log the name of all the people in house Lannisters (index 1) one by one
for (const people of got.houses[1].people) {
  console.log(people.name, "lannister");
}
// 6. Log the description of all the people in house Lannisters (index 1) one by one
for (const people of got.houses[1].people) {
  console.log(people.description, "desc");
}
...
for (const people of got.houses[1].people) {
  console.log(
    `I am ${people.name} and my bio is ${people.description}`,
    "-lannisters"
  );
}
...
for (const house of got.houses) {
  for (const people of house.people) {
...
if (allDescription > 30) {
  for (let i = 0; i > allDescription; i++);
