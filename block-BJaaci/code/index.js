...
console.log(movies[1].genres[movies[0].genres.length - 2]);
...
// let firstActor = "Rachel McAdams";
// firstActor === `Rachel McAdams` ? true : false;
...
let SecondActor = `Natalie Portman`;
...
for (let movie of movies) {
  if (movie === "Hannah") console.log(movie.title);
}
...
for (let i = 0; i < movies.length; i++) {
  if (movies[i].actors.length > 1) {
    console.log(movies[i].title, "More than 1 actor");
  }
}
